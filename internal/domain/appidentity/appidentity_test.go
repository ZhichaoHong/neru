...
func TestMatches(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			got := appidentity.Matches(testCase.configured, testCase.identity)
			if got != testCase.want {
				t.Errorf(
					"Matches(%q, %q) = %v, want %v",
					testCase.configured, testCase.identity, got, testCase.want,
				)
			}
		})
	}
...
func TestMatchesAny(t *testing.T) {
	...
	configured := []string{safariBundleID, chromeExe}
