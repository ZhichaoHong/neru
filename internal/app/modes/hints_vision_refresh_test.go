...
// TestActivateHints_RefreshClearsTheOverlayOnlyForVision pins the ordering the
// vision strategy depends on and the flash-free redraw every other strategy
// keeps.
//
// A refresh deliberately leaves the previous activation's labels on screen so
// the redraw does not blink. Vision reads the screen, so for it those labels are
// input: they come back as elements of their own, and they cover the text they
// point at. The accessibility tree cannot see the overlay, so nothing there
// needs clearing and the blink would be paid for nothing.
func TestActivateHints_RefreshClearsTheOverlayOnlyForVision(t *testing.T) {
	t.Parallel()
