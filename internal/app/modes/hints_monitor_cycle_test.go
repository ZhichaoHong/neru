...
func newCycleHarness(
	...
	cfg.Hints.CaptureScope = domain.CaptureScopeWindow
...
func TestHintsMonitorCycle_ClearsTheSourceCollectionToo(t *testing.T) {
	...
	setErr := harness.handler.hints.Context.SetHints(seeded)
	if setErr != nil {
...
// returned; see hint_service.go), so it arrives here as an empty answer and takes
