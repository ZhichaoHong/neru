...
// — darwin, Windows, and a Linux session whose compositor exposes a
// focus-change signal — the platform is asked at most until it first fires.
// Where none does, a GNOME/Mutter session, it is asked once per mode opened,
// and the consequence documented in docs/CROSS_PLATFORM.md holds there:
// overrides settle when the mode opens rather than the instant the user
// switches apps.
//
// Caller must hold h.mu.
