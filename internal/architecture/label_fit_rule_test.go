...
const (
	...
	// what a user sees — a factor moved across a multiplication — is not reported
	// as drift, while every substantive change to the rule remains a difference of
	// whole points.
...
// Objective-C source is read into something this test can evaluate — its
// multipliers and its floor read out of the same file, so a constant edited on
// its own is a disagreement and not a silent pass — and its answer is compared
...
var labelFitOperands = map[string]func(labelFitInputs) float64{
	...
	"cellRect.size.width":     func(inputs labelFitInputs) float64 { return inputs.cellWidth },
	"cellRect.size.height":    func(inputs labelFitInputs) float64 { return inputs.cellHeight },
}
...
func parseNativeLabelFitGlyphCount(body string) (string, string) {
	...
	if len(matches) != 1 {
		return "", fmt.Sprintf(
			...
			labelFitNativeMethod, len(matches),
		)
	}
...
func parseNativeLabelFitBase(body, glyphCountName string) (string, string, string) {
	...
	if len(found) != 1 {
		return "", "", fmt.Sprintf(
			...
			labelFitNativeMethod, len(found),
		)
	}
...
func parseNativeLabelFitLimits(body, fittedName string) ([]nativeLabelFitLimit, string) {
	...
	if len(matches) != labelFitLimitCount {
		return nil, fmt.Sprintf(
			...
			labelFitNativeMethod, len(matches), fittedName, fittedName, labelFitLimitCount,
		)
	}
...
func validateNativeLabelFitRule(rule nativeLabelFitRule) string {
	...
	comparisons := []nativeRuleComparison{rule.emptyGuard}
	...
	for _, token := range tokens {
		...
		if _, parseErr := strconv.ParseFloat(token, 64); parseErr == nil {
			continue
		}
