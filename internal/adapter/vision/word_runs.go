...
// its neighbor: "core" against "Config" differs by the cap height, so demanding
...
func groupTextLines(words []DetectedRegion) [][]DetectedRegion {
	...
	for _, word := range words {
		...
		for index := range lines {
			if !sameTextLine(spans[index], word.Bounds) {
				continue
			}

			lines[index] = append(lines[index], word)
			spans[index] = spans[index].Union(word.Bounds)
			joined = true
