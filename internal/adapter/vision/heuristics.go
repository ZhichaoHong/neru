...
// regionOutranks reports whether region survives non-maximum suppression
// against other.
//
...
func regionOutranks(region, other DetectedRegion) bool {
	if region.Score != other.Score {
		return region.Score > other.Score
	}

	regionArea := region.Bounds.Dx() * region.Bounds.Dy()

	otherArea := other.Bounds.Dx() * other.Bounds.Dy()
	if regionArea != otherArea {
		return regionArea > otherArea
	}

	if region.Bounds.Min.Y != other.Bounds.Min.Y {
		return region.Bounds.Min.Y < other.Bounds.Min.Y
	}

	return region.Bounds.Min.X < other.Bounds.Min.X
}
