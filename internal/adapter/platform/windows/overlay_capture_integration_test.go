...
// releaseCall runs a GDI cleanup call whose results say nothing this test can
// act on: it is already tearing the grab down, and the pixels it came for are
// either read by then or the test has failed.
func releaseCall(proc *windows.LazyProc, args ...uintptr) {
...
func grabScreenRect(t *testing.T, rect image.Rectangle, useCaptureBLT bool) []byte {
	...
	defer releaseCall(procCaptureReleaseDC, 0, screenDC)
	...
	defer releaseCall(procCaptureDeleteDC, memDC)
	...
	defer releaseCall(procCaptureDeleteObject, bitmap)

	previous, _, _ := procCaptureSelectObject.Call(memDC, bitmap)
	defer releaseCall(procCaptureSelectObject, memDC, previous)
