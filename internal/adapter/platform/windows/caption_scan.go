	...
	for _, fromRight := range []bool{true, false} {
		...
		for _, row := range lines {
			for offset := 0; offset < span; offset += step {
				column := area.Min.X + offset
				if fromRight {
					column = area.Max.X - 1 - offset
				}

				code, ok := probe(image.Pt(column, row))
				if !ok {
					return nil, false
				}
				...
				if _, seen := seeds[code]; !seen {
					seeds[code] = image.Pt(column, row)
				}
			}
...
func growCaptionButton(
	...
	left, probed := captionRunEnd(func(offset int) image.Point {
	...
	right, probed := captionRunEnd(func(offset int) image.Point {
		return image.Pt(seed.X+offset, seed.Y)
	...
	top, probed := captionRunEnd(func(offset int) image.Point {
	...
	bottom, probed := captionRunEnd(func(offset int) image.Point {
		return image.Pt(seed.X, seed.Y+offset)
...
// answers with code, bounded by maxOffset. pointAt maps an offset to the point
// to probe, which is what lets one function walk all four directions.
func captionRunEnd(
	pointAt func(offset int) image.Point,
	code hitCode,
	maxOffset int,
	step int,
	probe captionProbe,
	...
	for outside-inside > 1 {
		middle := inside + (outside-inside)/2 //nolint:mnd

		got, ok := probe(pointAt(middle))
		if !ok {
			return 0, false
		}
...
func scaleLogical(logical int, scale float64) int {
	...
	return max(1, int(math.Round(float64(logical)*scale)))
}
