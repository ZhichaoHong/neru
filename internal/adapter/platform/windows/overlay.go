...
// FontWeight is the weight a text draw renders at. It exists because the
// weight is per label rather than per backend: badges are bold so they read
// against arbitrary content underneath, while grid labels are regular so a
// three-character coordinate fits its cell. The darwin backend draws the same
// split.
//
// The values are FW_NORMAL and FW_BOLD, which DirectWrite happens to share, so
// one number serves CreateFontW and CreateTextFormat.
type FontWeight int
...
type OverlayWindow struct {
	...
	// a capturable window. createHWNDLocked reapplies it.
...
// it cannot interleave with a createHWNDLocked that is replacing the handle.
