...
func TestPresentCopyReplacesTheDestination(t *testing.T) {
	...
	if err := surface.context.hresult(
		"ID2D1RenderTarget::EndDraw", vtblD2DRTEndDraw, 0, 0); err != nil {
		t.Fatalf("copy: %v", err)
	}
...
func newReadableBitmap(t *testing.T, s *dcompSurface, side int, options uint32) comObject {
	t.Helper()
...
func clearBitmap(t *testing.T, s *dcompSurface, target comObject, color d2dColorF) {
	t.Helper()

	s.context.call(vtblD2DDCSetTarget, objArg(target))
	s.context.call(vtblD2DRTBeginDraw)
	s.context.call(vtblD2DRTClear, ptrArg(unsafe.Pointer(&color)))

	if err := s.context.hresult(
		"ID2D1RenderTarget::EndDraw", vtblD2DRTEndDraw, 0, 0); err != nil {
...
func firstPixel(t *testing.T, readback, source comObject) [4]byte {
	...
	if err := readback.hresult(
		...
		d2dMapOptionsRead, ptrArg(unsafe.Pointer(&mapped))); err != nil {
		t.Fatalf("Map: %v", err)
	}
