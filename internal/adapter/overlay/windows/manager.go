...
func (m *Manager) SetSharingType(hide bool) {
	...
	// One window per display, so each is told separately.
	for _, win := range m.monitorWins {
		if win == nil {
			continue
		}

		m.reportAffinity("monitor-select", hide, win.SetExcludedFromCapture(hide))
	}
...
func (m *Manager) newOverlayWindowAt(
