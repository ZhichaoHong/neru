...
func (o *winOverlay) DrawHints(
	...
	for _, hint := range hintsSlice {
		...
		if style.BoundaryHighlightEnabled() {
			...
			if bw := float64(max(style.BoundaryBorderWidth(), 0)); bw > 0 {
				o.window.StrokeRoundedRect(
					element, bdr, badge.ParseHexARGB(style.BoundaryBorderColor()), bw*o.scale(),
				)
			}
		}
		...
		//
		// The font is scaled here rather than at the text call, because every
		// dimension below is derived from it: a badge sized from the logical font
		// would clip the glyphs drawn at the scaled one.
		fontSize := float64(max(style.FontSize(), 1)) * o.scale()
		...
		// The window's own text call rather than drawTextCentered, because
		// fontSize was already scaled above to size the badge from it, and the
		// helper would scale it a second time. The family arrives resolved here
		// for the same reason drawTextCentered documents, and this is the
		// surface that redraws on every keystroke. The window paints commands
		// in the order they were queued, so this hint's fill, arrow, stroke and
		// label land as one unit over every hint queued before it.
...
func (o *winOverlay) DrawRecursiveGrid(
	...
	for idx, cell := range cellRects {
		...
		if idx < len(keyRunes) {
			...
			if style.ShowLabelIn(cell, o.scale()) {
	...
	if virtualPointer.Visible {
		...
		//
		// The box scales because drawTextCentered scales the font it is sized
		// from; left logical, it would clip the glyph on a scaled monitor.
		halfSize := max(int(fontSize*o.scale())/2, 1) //nolint:mnd
