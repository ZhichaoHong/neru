...
func TestMergeCaptionButtonsSynthesizesMissingButtons(t *testing.T) {
	...
	existing := []winElement{
		...
		{bounds: image.Rect(500, 4, 524, 28), role: uiaControlButton, name: "Avatar", clickable: true},
	}

	got := mergeCaptionButtons(existing, buttons)

	want := []winElement{
		{bounds: image.Rect(600, 0, 645, 32), role: uiaControlButton, name: "Minimize", clickable: true},
		{bounds: image.Rect(645, 0, 690, 32), role: uiaControlButton, name: "Maximize", clickable: true},
		{bounds: image.Rect(690, 0, 735, 32), role: uiaControlButton, name: "Close", clickable: true},
	}
...
func TestMergeCaptionButtonsSkipsPublishedButtons(t *testing.T) {
	...
	existing := []winElement{
		{bounds: image.Rect(1608, 2, 1677, 34), role: uiaControlButton, name: "Minimize", clickable: true},
		{bounds: image.Rect(1677, 2, 1746, 34), role: uiaControlButton, name: "Maximize", clickable: true},
	}
