...
const (
	...
	// stops a configuration asking for 100000. The wheel path clamps a large
	// delta for the same reason, at the signed short WM_MOUSEWHEEL carries.
	maxScrollSteps = 200
)
...
func horizontalScrollPatternAt(automation unsafe.Pointer, point image.Point) unsafe.Pointer {
	...
	for range maxScrollAncestorWalk {
		...
		if failed(comCall(walker, vtWalkerGetParent, uintptr(current), uintptr(unsafe.Pointer(&parent)))) {
			parent = nil
		}
