...
//
// Navigation carries the same cache request the bulk enumeration used, so a part
// arrives with the four properties extractWinElement reads already attached
...
func hiddenParts(
	...
	hresult := comCall(
		walker,
		vtWalkerGetFirstChildBuildCache,
		uintptr(wrapper),
		uintptr(cache),
	...
	for child != nil {
		...
		if failed(comCall(
			walker,
			vtWalkerGetNextSiblingBuildCache,
			uintptr(child),
			uintptr(cache),
