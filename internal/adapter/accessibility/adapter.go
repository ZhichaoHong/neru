...
//
// It holds no configuration. Everything a collection needs from config arrives
// on the ports.ElementFilter it is handed, and the roles it hints against live
// in the client's own vocabulary - so a reload has nothing here to refresh, and
// therefore nothing here to forget.
type Adapter struct {
	logger *zap.Logger
	client ax.Client
}
...
func NewAdapter(logger *zap.Logger, client ax.Client) *Adapter {
	return &Adapter{
		logger: logger,
		client: client,
	}
