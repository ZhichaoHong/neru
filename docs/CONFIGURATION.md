...
| Windows | Executable name (e.g. `chrome.exe`) | Task Manager → Details tab → *Name* column, or `Get-Process chrome \| Select-Object Path` |
...
**Getting the hotkey to fire at all is the harder half on a remote client.** A full-screen RDP or Citrix session captures the keyboard for the remote machine, so a trigger delivered by a user-mode keyboard hook - Neru's own hotkeys, or a remapper like kanata's default Windows build - is never seen locally and hints never activate. Two things work: run the session in a window rather than full-screen, which is enough for most people and costs nothing, or drive the trigger from a driver-level remapper (kanata's Interception build, for instance) that sits below the client's hook. If the trigger is a Windows-key combination there is a third option - `keyboardhook:i:1` in the `.rdp` file, or Local Resources → Keyboard → "On this computer", applies Windows key combinations locally even in full-screen - but it does nothing for ordinary keys.
